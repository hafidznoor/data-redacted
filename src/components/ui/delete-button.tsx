...
> & {
  ...
  onCancel?: () => void;
  /**
   * Local addition to the upstream rare-ui component: the labels were
   * hardcoded to "Delete"/"Deleted"/"Kept", which reads wrong for a redact
   * confirmation and cannot be translated. Defaults preserve the original.
   */
  labels?: {
    action?: string;
    confirm?: string;
  ...
  return (
    ...
    >
      ...
      <span role="status" aria-live="polite" className="sr-only">
        {status === "deleted" ? (labels?.confirmed ?? "Deleted") : status === "kept" ? (labels?.cancelled ?? "Kept") : ""}
      </span>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            />
            <Circle label={labels?.confirm ?? "Confirm delete"} onClick={() => resolve("deleted")}>
              <path d="M4 12.5 9.5 18 20 7" stroke={ACCENT} />
            </Circle>
            <Circle label={labels?.cancel ?? "Cancel"} onClick={() => resolve("kept")}>
